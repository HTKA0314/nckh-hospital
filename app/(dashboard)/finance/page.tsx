  ...
  return (
    <div className="space-y-3 text-slate-800">
      ...
      <PageHeader
        ...
        description="Theo dõi tình hình giải ngân, thanh toán và quyết toán kinh phí đề tài"
        actions={
          <button
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs px-4 py-2.5 flex flex-wrap items-center gap-2.5">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-1.5 rounded-lg border border-slate-300 focus:border-[#0A6EBD] focus:ring-1 focus:ring-[#0A6EBD] text-xs outline-none bg-white transition shadow-xs"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length === 0 ? (
                <TableEmptyState
                  ...
                  description="Không tìm thấy dữ liệu tài chính nào phù hợp."
                />
              ) : (
                pagedProjects.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/80 transition">
                      ...
                      <td className="px-4 py-3.5 text-center whitespace-nowrap align-middle">
                        <Link
                          href={`/projects/${p.id}`}
                          className="px-2.5 py-1.5 bg-[#EBF4FC] hover:bg-[#D8ECF9] text-[#0A6EBD] rounded-lg border border-[#B8D7F5] font-bold text-[11px] inline-flex items-center gap-1 transition shadow-2xs"
